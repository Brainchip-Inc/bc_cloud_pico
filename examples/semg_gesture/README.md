...
builder and all inline plotting code live in the notebook itself, so it can be read, run and
modified in one place. The one exception is the closing literature comparison, which is embedded
as a static image generated by `img/make_accuracy_plot.py`.
...
|- img/
    |- accuracy_vs_parameters.png         # The literature comparison figure, as shown in the notebook
    |- make_accuracy_plot.py              # Regenerates that figure from the two CSVs above
```
...
| `img/accuracy_vs_parameters.png` | The closing comparison figure, embedded in the notebook as a static image and generated by `img/make_accuracy_plot.py` |
